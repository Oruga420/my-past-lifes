...
import { generateDecadeImage, generateStoryAndNarration } from './services/geminiService';
...
type NarrationStatus = 'pending' | 'done' | 'error';
...
function App() {
    ...
    useEffect(() => {
        // Fix for webkitAudioContext TypeScript error by casting window to any
        const context = new ((window as any).AudioContext || (window as any).webkitAudioContext)({ sampleRate: 24000 });
    ...
    const processDecade = async (decade: string) => {
        ...
        try {
            ...
            // Update state to show image is done and narration is starting
            setGeneratedImages(prev => ({
                ...prev,
                [decade]: { status: 'done', url: resultUrl, narrationStatus: 'pending' },
            }));

            // Step 2: Generate the story and narration
            try {
                const { story, audioData } = await generateStoryAndNarration(resultUrl, decade);
                setGeneratedImages(prev => ({
                    ...prev,
                    [decade]: { ...prev[decade], story, audioData, narrationStatus: 'done' },
                }));
            } catch (narrationError) {
                console.error(`Failed to generate narration for ${decade}:`, narrationError);
                 setGeneratedImages(prev => ({
                    ...prev,
                    [decade]: { ...prev[decade], narrationStatus: 'error' },
                }));
            }
        } catch (err) {
    ...
    const handleDownloadAlbum = async () => {
        ...
        try {
            // FIX: Explicitly type the destructured `image` parameter in the filter and reduce callbacks to resolve type inference issues.
    ...
    const handlePlayNarration = async (decade: string) => {
        if (!audioContext) return;
        if (activeAudioSource) {
            activeAudioSource.stop();
        }

        const audioData = generatedImages[decade]?.audioData;
        if (!audioData) return;
        ...
        try {
            ...
            alert("Sorry, there was an error playing the narration.");
        }
    };
        ...
        <main className="bg-black text-neutral-200 min-h-screen w-full flex flex-col items-center justify-center p-4 pb-24 overflow-hidden relative">
            ...
            <div className="z-10 flex flex-col items-center justify-center w-full h-full flex-1 min-h-0">
                <div className="text-center mb-10">
                    <h1 className="text-6xl md:text-8xl font-caveat font-bold text-neutral-100">My Past Lifes</h1>
                    <p className="font-permanent-marker text-neutral-300 mt-2 text-xl tracking-wide">how did you live in your past lifes</p>
                </div>
                                ...
                                {DECADES.map((decade) => (
                                    <div key={decade} className="flex justify-center">
                                         <PolaroidCard
                                            ...
                                            story={generatedImages[decade]?.story}
                                            narrationStatus={generatedImages[decade]?.narrationStatus}
