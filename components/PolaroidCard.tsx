...
interface PolaroidCardProps {
    ...
    onPlayNarration?: (caption: string) => void;
    isMobile?: boolean;
}
...
const NarrationLoadingSpinner = () => (
    <div className="flex items-center gap-2 text-neutral-600 dark:text-neutral-700 text-xs">
...
const PolaroidCard: React.FC<PolaroidCardProps> = ({ imageUrl, caption, status, error, story, narrationStatus, dragConstraintsRef, onShake, onDownload, onPlayNarration, isMobile }) => {
    ...
    const cardInnerContent = (
        <>
            ...
            <div className="absolute bottom-4 left-4 right-4 px-2 flex flex-col items-center justify-end h-20">
                {status === 'done' && imageUrl && narrationStatus === 'pending' && <NarrationLoadingSpinner />}
                {story && (
                    <p className="text-[11px] leading-tight sm:text-xs sm:leading-snug text-neutral-700 dark:text-neutral-800 mb-1 text-center font-sans italic">
                        &ldquo;{story}&rdquo;
                    </p>
                )}
                <div className="flex items-center justify-center gap-2 mt-1">
                    {narrationStatus === 'done' && onPlayNarration && (
                        <button
                            onClick={(e) => { e.stopPropagation(); onPlayNarration(caption); }}
                            className="text-black/60 dark:text-black/60 hover:text-black dark:hover:text-black transition-colors duration-200"
                            aria-label={`Play story for ${caption}`}
                        >
